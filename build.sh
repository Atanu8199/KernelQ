...
Upload() {
	...
	-F "disable_web_page_preview=true" \
	-F "parse_mode=html" \
	-F caption="Build Fail in : $((DIFF / 60)) minute(s) and $((DIFF % 60)) second(s)"
    fi
}
...
MakeZip() {
    ...
    cp -af $MainPath/out/arch/arm64/boot/dtbo.img $Any
    cp -af anykernel-real.sh anykernel.sh
    sed -i "s/kernel.string=.*/kernel.string=$KERNEL_NAME-$HeadCommit by ${KBUILD_BUILD_USER}/g" anykernel.sh
    zip -r $MainPath/"${Compiler}_R-OSS_$ZIP_KERNEL_VERSION-$KERNEL_NAME-$TIME.zip" * -x .git .git/**\* ./.git ./anykernel-real.sh ./.gitignore ./LICENSE ./README.md ./*.zip
    cd $MainPath
}
