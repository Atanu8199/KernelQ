...
static int G_WRDMA_IonCt[
	ISP_CAMSV0_IDX - ISP_CAM_A_IDX][
	_dma_max_wr_*_ion_keep_max_];
static int G_WRDMA_IonFd[
	ISP_CAMSV0_IDX - ISP_CAM_A_IDX][
	_dma_max_wr_*_ion_keep_max_];
static struct ion_handle *G_WRDMA_IonHnd[
	ISP_CAMSV0_IDX - ISP_CAM_A_IDX][
	_dma_max_wr_*_ion_keep_max_];
/* protect G_WRDMA_IonHnd & G_WRDMA_IonFd */
static spinlock_t SpinLock_IonHnd[
	ISP_CAMSV0_IDX - ISP_CAM_A_IDX][
	_dma_max_wr_];
...
static struct T_ION_TBL gION_TBL[ISP_DEV_NODE_NUM] = {
	...
	{ISP_CAM_A_IDX,
		(int *)G_WRDMA_IonCt[
			ISP_CAM_A_IDX - ISP_CAM_A_IDX],
		(int *)G_WRDMA_IonFd[
			ISP_CAM_A_IDX - ISP_CAM_A_IDX],
		(struct ion_handle **)G_WRDMA_IonHnd[
			ISP_CAM_A_IDX - ISP_CAM_A_IDX],
		(spinlock_t *)SpinLock_IonHnd[
			ISP_CAM_A_IDX - ISP_CAM_A_IDX]},
	{ISP_CAM_B_IDX,
		(int *)G_WRDMA_IonCt[
			ISP_CAM_B_IDX - ISP_CAM_A_IDX],
		(int *)G_WRDMA_IonFd[
			ISP_CAM_B_IDX - ISP_CAM_A_IDX],
		(struct ion_handle **)G_WRDMA_IonHnd[
			ISP_CAM_B_IDX - ISP_CAM_A_IDX],
...
struct ISP_IRQ_ERR_WAN_CNT_STRUCT {
	...
	unsigned int m_err_int_cnt[
		ISP_IRQ_TYPE_AMOUNT][ISP_ISR_MAX_NUM];
	/* cnt for each warning int # */
	unsigned int m_warn_int_cnt[
		ISP_IRQ_TYPE_AMOUNT][ISP_ISR_MAX_NUM];
	/* mark for err int, where its cnt > threshold */
	unsigned int m_err_int_mark[
		ISP_IRQ_TYPE_AMOUNT];
	/* mark for warn int, where its cnt > threshold */
	unsigned int m_warn_int_mark[
		ISP_IRQ_TYPE_AMOUNT];
	unsigned long m_int_usec[
		ISP_IRQ_TYPE_AMOUNT];
};
...
struct ISP_IRQ_INFO_STRUCT {
	...
	unsigned int    MarkedFlag[
		ISP_IRQ_TYPE_AMOUNT][
		ISP_IRQ_ST_AMOUNT][IRQ_USER_NUM_MAX];
	/* time for marking a specific interrupt */
	unsigned int    MarkedTime_sec[
		ISP_IRQ_TYPE_AMOUNT][32][
		IRQ_USER_NUM_MAX];
	/* time for marking a specific interrupt */
	unsigned int    MarkedTime_usec[
		ISP_IRQ_TYPE_AMOUNT][32][
		IRQ_USER_NUM_MAX];
...
#define CAM_AMOUNT		(ISP_IRQ_TYPE_INT_CAM_C_ST-\
...
/**
 *   for irq used,keep log until
 *   IRQ_LOG_PRINTER being involked,
 *   limited:
...
#define ISP_RD32_TG_CAM_FRM_CNT(IrqType, reg_module) ({ \
	...
	_regVal = ISP_RD32( \
		CAM_REG_TG_INTER_ST(reg_module)); \
 ...
 ************************************************/
#if defined(ISP_MET_READY)
int MET_Event_Get_BPP(enum _isp_dma_enum_ dmao,
	unsigned int reg_module)
{
	unsigned int fmt_sel = ISP_RD32(
		CAM_REG_CTL_FMT_SEL(reg_module));
	int ret = 0;
...
void CAMSYS_MET_Events_Trace(bool enter,
	u32 reg_module,
...
ISP_GetIRQState(unsigned int type,
	unsigned int stType,
	unsigned int userNumber,
	unsigned int stus)
{
	unsigned int ret;
	/* old: unsigned int flags;*/
	...
	spin_lock_irqsave(&(IspInfo.SpinLockIrq[
		type]), flags);
	ret = (IspInfo.IrqInfo.Status[
		type][stType][userNumber] & stus);
	spin_unlock_irqrestore(&(
		IspInfo.SpinLockIrq[type]), flags);
	/*  */
	return ret;
}
...
static void ISP_DumpDmaDeepDbg(enum ISP_IRQ_TYPE_ENUM module)
{
	...
	enum ISP_DEV_NODE_ENUM regModule;
	...
	dmaerr[_imgo_] = (unsigned int)ISP_RD32(
		CAM_REG_IMGO_ERR_STAT(regModule));
	dmaerr[_rrzo_] = (unsigned int)ISP_RD32(
		CAM_REG_RRZO_ERR_STAT(regModule));
	dmaerr[_aao_] = (unsigned int)ISP_RD32(
		CAM_REG_AAO_ERR_STAT(regModule));
	dmaerr[_afo_] = (unsigned int)ISP_RD32(
		CAM_REG_AFO_ERR_STAT(regModule));
	dmaerr[_lcso_] = (unsigned int)ISP_RD32(
		CAM_REG_LCSO_ERR_STAT(regModule));
	dmaerr[_ufeo_] = (unsigned int)ISP_RD32(
		CAM_REG_UFEO_ERR_STAT(regModule));
	dmaerr[_bpci_] = (unsigned int)ISP_RD32(
		CAM_REG_BPCI_ERR_STAT(regModule));
	dmaerr[_lsci_] = (unsigned int)ISP_RD32(
		CAM_REG_LSCI_ERR_STAT(regModule));
	dmaerr[_pdo_] = (unsigned int)ISP_RD32(
		CAM_REG_PDO_ERR_STAT(regModule));
	dmaerr[_pso_] = (unsigned int)ISP_RD32(
		CAM_REG_PSO_ERR_STAT(regModule));
	dmaerr[_pdi_] = (unsigned int)ISP_RD32(
		CAM_REG_PDI_ERR_STAT(regModule));

	dmaerr[_lmvo_] = (unsigned int)ISP_RD32(
		CAM_REG_LMVO_ERR_STAT(regModule));
	dmaerr[_flko_] = (unsigned int)ISP_RD32(
		CAM_REG_FLKO_ERR_STAT(regModule));
	dmaerr[_rsso_] = (unsigned int)ISP_RD32(
		CAM_REG_RSSO_A_ERR_STAT(regModule));
	dmaerr[_ufgo_] = (unsigned int)ISP_RD32(
		CAM_REG_UFGO_ERR_STAT(regModule));
	dmaerr[_rawi_] = (unsigned int)ISP_RD32(
		CAM_UNI_REG_RAWI_ERR_STAT(ISP_UNI_A_IDX));
	...
	IRQ_LOG_KEEPER(module, m_CurrentPPB, _LOG_ERR,
	    "%s:IMGO:0x%x,RRZO:0x%x,AAO=0x%x,AFO=0x%x,LCSO=0x%x,UFEO=0x%x,PDO=0x%x,PSO=0x%x\n,EISO=0x%x,RSSO=0x%x,UFGO=0x%x,FLKO=0x%x DMA_DBG_SEL=0x%x TOP_DBG_PORT=0x%x\n",
			...
			(unsigned int)ISP_RD32(
				CAM_UNI_REG_TOP_DBG_PORT(ISP_UNI_A_IDX)));

	/* CAM_UNI_TOP_DBG_SET [15:12] = 4'd0; for smi dbg dump */
	ISP_WR32(CAM_UNI_REG_TOP_DBG_SET(ISP_UNI_A_IDX),
		(ISP_RD32(
			CAM_UNI_REG_TOP_DBG_SET(ISP_UNI_A_IDX)) &
			0xFFFF0FFF));

	ISP_WR32(CAM_REG_DMA_DEBUG_SEL(regModule),
		0x00080403);
...
static inline void ISP_Reset(int module)
{
	...
	case ISP_CAMSV5_IDX: {
		...
		/* SW_RST: 1 */
		ISP_WR32(
			CAMSV_REG_SW_CTL(module), 0x4);
		/* SW_RST: 0 */
		ISP_WR32(
			CAMSV_REG_SW_CTL(module), 0x0);
		/* IMGO_RST_TRIG: 1 */
		ISP_WR32(
			CAMSV_REG_SW_CTL(module), 0x1);
		/* Polling IMGO_RST_ST to 1 */
		while (ISP_RD32(
			CAMSV_REG_SW_CTL(module)) != 0x3) {
	...
	case ISP_UNI_A_IDX: {
		...
		ISP_WR32(
			CAM_UNI_REG_TOP_SW_CTL(module), 0x0);
		break;
	}
	default:
 ...
 * if need write to other modules,
...
/* isr dbg log , sw isr response counter,
 * +1 when sw receive 1 sof isr.
 */
...
static long ISP_Buf_CTRL_FUNC(unsigned long Param)
{
	...
	if (copy_from_user(&rt_buf_ctrl,
		...
		case ISP_RT_BUF_CTRL_CLEAR:
			...
			memset(
				(void *)IspInfo.IrqInfo.LastestSigTime_usec[
				rt_buf_ctrl.module],
				0, sizeof(unsigned int) * 32);
			memset(
				(void *)IspInfo.IrqInfo.LastestSigTime_sec[
				rt_buf_ctrl.module],
				0, sizeof(unsigned int) * 32);
			/* remove, cause clear will be involked
			 * only when current module r totally stopped
			 */
			...
			memset((char *)&pstRTBuf[
				rt_buf_ctrl.module]->ring_buf[rt_dma],
				0x00, sizeof(
			...
			case ISP_IRQ_TYPE_INT_CAM_C_ST:
				if ((pstRTBuf[
					...
					#endif
					g_ISPIntStatus[
						rt_buf_ctrl.module].ispIntErr
						= 0;
					g_ISPIntStatus[
						rt_buf_ctrl.module].ispInt3Err
						= 0;
					g_ISPIntStatus_SMI[
						rt_buf_ctrl.module].ispIntErr
						= 0;
					...
					pstRTBuf[rt_buf_ctrl.module]->dropCnt
						= 0;
					pstRTBuf[rt_buf_ctrl.module]->state
						= 0;
				}
			...
			case ISP_IRQ_TYPE_INT_CAMSV_5_ST:
				if (pstRTBuf[rt_buf_ctrl.module]->ring_buf[
					...
					g_ISPIntStatus_SMI[
						...
						= 0;
					pstRTBuf[
						rt_buf_ctrl.module]->dropCnt
						= 0;
					pstRTBuf[
						rt_buf_ctrl.module]->state
						= 0;
				}
...
static int ISP_WaitIrq(struct ISP_WAIT_IRQ_STRUCT *WaitIrq)
{
...

	{
		...
		IspInfo.IrqInfo.MarkedFlag[WaitIrq->Type][
			...
			/* if the entry time of wait request
			 * after mark is before signal
			 */
...
EXIT:
	/* 4. clear mark flag
	 * reset marked time
	 * reset time related infor and
	 * passedby signal count
	 */
	spin_lock_irqsave(&(IspInfo.SpinLockIrq[
		WaitIrq->Type]), flags);
...
static void ISP_ion_free_handle_by_module(unsigned int module)
{
	...
	for (i = 0; i < _dma_max_wr_; i++) {
		...
		for (j = 0; j < _ion_keep_max_ ; j++) {
			...
			/*can't in spin_lock*/
			ISP_ion_free_handle(pIon_client,
				p_IonHnd);
		}
	}
}
 ...
 *
 *******************************************/
static long ISP_ioctl(struct file *pFile,
	unsigned int Cmd, unsigned long Param)
{
	...
	/*  */
	pUserInfo = (struct ISP_USER_INFO_STRUCT *)(
			pFile->private_data);
	...
	case ISP_GET_DROP_FRAME:
		if (copy_from_user(&DebugFlag[0],
			...
			DebugFlag[1] = FrameStatus[
				DebugFlag[0]];
			spin_unlock_irqrestore(&(
				IspInfo.SpinLockIrq[DebugFlag[0]]),
				flags);
	...
	case ISP_GET_SUPPORTED_ISP_CLOCKS:
			{
				...
				memset((void *)&ispclks, 0x0,
					sizeof(struct ISP_CLK_INFO));

				ispclks.clklevelcnt =
					(unsigned char)ISP_SetPMQOS(
						E_CLK_SUPPORTED,
						ISP_IRQ_TYPE_INT_CAM_A_ST,
						(unsigned int *)
							ispclks.clklevel);
	...
	case ISP_DFS_CTRL:
		{
			...
			if (copy_from_user(&dfs_ctrl,
					(void *)Param,
					sizeof(unsigned int)) == 0) {
				ISP_SetPMQOS(E_CLK_CLR,
						ISP_IRQ_TYPE_INT_CAM_A_ST,
						NULL);
			} else {
				LOG_NOTICE("Cmd(%d) copy fail\n", Cmd);
	...
	case ISP_DFS_UPDATE:
		{
			...
			if (copy_from_user(&dfs_update,
					(void *)Param,
					sizeof(unsigned int)) == 0) {
				ISP_SetPMQOS(E_CLK_UPDATE,
						ISP_IRQ_TYPE_INT_CAM_A_ST,
						&dfs_update);
			} else {
				LOG_NOTICE("Cmd(%d) copy fail\n", Cmd);
	...
	case ISP_SET_PM_QOS_INFO:
		{
			...
			if (copy_from_user(&pm_qos_info,
					(void *)Param,
					sizeof(struct ISP_PM_QOS_INFO_STRUCT))
					== 0) {
				ISP_SetPMQOS(E_BW_UPDATE,
					pm_qos_info.module,
	...
	case SV_SET_PM_QOS_INFO:
		{
			struct ISP_SV_PM_QOS_INFO_STRUCT pm_qos_info;

			if (copy_from_user(&pm_qos_info,
				(void *)Param,
				sizeof(struct ISP_SV_PM_QOS_INFO_STRUCT))
				== 0) {
				SV_SetPMQOS(E_BW_UPDATE,
					pm_qos_info.module,
					(unsigned int *)pm_qos_info.port_bw);
			} else {
				LOG_NOTICE("Cmd(%d) copy fail\n", Cmd);
	...
	case ISP_GET_GLOBAL_TIME:
		{
			...
			if (copy_from_user(hwTickCnt, (void *)Param,
				...
				archcounter_timesync_to_monotonic(
					hwTickCnt[0]);
				/* ns to us */
				do_div(globalTime[0], 1000);
#ifdef TS_BOOT_T
				/* ns */
				globalTime[1] =
				archcounter_timesync_to_boot(hwTickCnt[0]);
				/* ns to us */
				do_div(globalTime[1], 1000);
#endif
	...
	case ISP_ION_FREE:
		if (copy_from_user(&IonNode, (void *)Param,
			 ...
			 * divid it into many buffers,
			 * the ion FD of these buffers is the same,
			 * so we must check there has
			 * no users take this memory
			 */
	...
	case ISP_CQ_SW_PATCH: {
		...
		if (copy_from_user(&multiRAWConfig,
			(void *)Param,
	...
	case ISP_SET_SEC_DAPC_REG:
		if (copy_from_user(Dapc_Reg, (void *)Param,
			...
			if (Dapc_Reg[1] == MTRUE) {
				sec_on = Dapc_Reg[1];
				lock_reg.CAM_REG_CTL_EN[Dapc_Reg[
					0]] = Dapc_Reg[2];
				lock_reg.CAM_REG_CTL_DMA_EN[Dapc_Reg[
					0]] = Dapc_Reg[3];
				lock_reg.CAM_REG_CTL_SEL[Dapc_Reg[
					0]] = Dapc_Reg[4];
	...
	unsigned int cmd, unsigned long arg)
{
	...
	case COMPAT_ISP_BUFFER_CTRL: {
		...
		ret = filp->f_op->unlocked_ioctl(
			filp, ISP_BUFFER_CTRL, (unsigned long)data);
	...
	struct file *pFile)
{
	...
	pFile->private_data = kmalloc(sizeof(
		struct ISP_USER_INFO_STRUCT), GFP_ATOMIC);
	if (pFile->private_data == NULL) {
		LOG_DBG(
			"ERROR: kmalloc failed, (process, pid, tgid)=(%s, %d, %d)\n",
	...
	/* reset backup regs*/
	memset(g_BkReg, 0, sizeof(
		struct _isp_bk_reg_t) * ISP_IRQ_TYPE_AMOUNT);
...
#endif
	/* Global timer enable */
	archcounter_timesync_init(MTRUE);
...
static inline void ISP_StopHW(int module)
{
	...
	/*500ms*/
	unsigned long long  timeoutMs = 500000000;
	char moduleName[128];
...
RESET:
	...
	while ((ISP_RD32(
		CAM_UNI_REG_TOP_SW_CTL(ISP_UNI_A_IDX)) &
			0x00000002) != 0x2) {
		sec = ktime_get();
		/* wait time>timeoutMs, break */
	...
	struct file *pFile)
{
	...
	LOG_DBG(
		...
		IspInfo.UserCount, current->comm,
		current->pid, current->tgid, pr_detect_count);
	LOG_DBG("CAMSYS clk gate(0x%x)\n", ISP_RD32(CAMSYS_REG_CG_CON));

	for (i = ISP_CAM_A_IDX; i < ISP_CAMSV0_IDX; i++) {
		/* Close VF when ISP_release */
		/* reason of close vf is to make sure camera
		 * can serve regular after previous abnormal exit
		 */
		...
		/* Reson: If twin is enabled,
		 * the twin module's DMX_SEL will be set to 1.
		 * It will encounter err when run
		 * single path and other module dmx_sel = 1
		 */
		if (!sec_on) {
	...
	if (sec_on) {
		memset(&lock_reg, 0,
			sizeof(struct isp_sec_dapc_reg));
		sec_on = 0;
	}
	...
	/* Disable clock.
	 *  1. clkmgr: G_u4EnableClockCount=0,
	 *		call clk_enable/disable
	 *  2. CCF: call clk_enable/disable every time
	 *     -> when IspInfo.UserCount,
	 *		disable all ISP clk
	 */
	spin_lock(&(IspInfo.SpinLockClock));
	...
	/*clear cam_qos*/
	{
		...
		value[0] = 0;
		ISP_SetPMQOS(E_CLK_CLR,
				ISP_IRQ_TYPE_INT_CAM_A_ST, NULL);
		for (j = ISP_IRQ_TYPE_INT_CAM_A_ST;
				j <= ISP_IRQ_TYPE_INT_CAM_C_ST; j++)
			ISP_SetPMQOS(E_BW_CLR, j, value);

		for (j = ISP_IRQ_TYPE_INT_CAMSV_0_ST;
				j <= ISP_IRQ_TYPE_INT_CAMSV_5_ST; j++)
			SV_SetPMQOS(E_BW_CLR, j, value);
	}
...
static int ISP_mmap(struct file *pFile, struct vm_area_struct *pVma)
{
	 ...
	 * pVma->vm_pgoff, pfn,
	 * pVma->vm_pgoff << PAGE_SHIFT, pVma->vm_start,
	 * pVma->vm_end, length);
	 */
...
)
{
	...
	if (regVal & 0x01) {
		...
		/*backup: frame CNT
		 * After VF enable,
		 * The frame count will be 0 at next VD;
...
static int ISP_resume(struct platform_device *pDev)
{
	...
	if (SuspnedRecord[module]) {
		...
		ISP_WR32(
			CAM_REG_TG_SEN_MODE(module), (regVal | 0x01));
		/*vf*/
		regVal = ISP_RD32(
			CAM_REG_TG_VF_CON(module));
		ISP_WR32(
...
static int __init ISP_Init(void)
{
	 ...
	 * sensor registers from device tree
	 */

	/* Don't use compatitble define
	 * in probe(). Otherwise,
	 * probe() of Seninf driver cannot be called.
	 */
	#if (SMI_LARB_MMU_CTL == 1)
	do {
		char *comp_str = NULL;
...
void IRQ_INT_ERR_CHECK_CAM(
	unsigned int WarnStatus,
	unsigned int ErrStatus,
	unsigned int warnTwo,
	enum ISP_IRQ_TYPE_ENUM module)
{
	/* ERR print */
	if (ErrStatus) {
				...
				unsigned int delayCheck)
{
	 ...
	 * ((ISP_RD32(CAM_REG_TG_SUB_PERIOD(module)) >> 8)
	 * & 0x1F) + 1;
	 */
	unsigned int i;
...
#if (TIMESTAMP_QUEUE_EN == 1)
void Irq_SW_TimeStamp(enum ISP_DEV_NODE_ENUM reg_module,
	unsigned int module,
	unsigned int irqDelay, unsigned int frmPeriod,
	unsigned long long  sec, unsigned long usec)
{
	...
	ISP_GetDmaPortsStatus(reg_module, IspInfo.TstpQInfo[
		module].DmaEnStatus);

	/* Prevent WCNT increase after
	 * ISP_CompensateMissingSofTime around P1_DON
	 * and FBC_CNT decrease to 0,
	 * following drop frame is checked becomes true,
	...
	if (FrameStatus[module] != CAM_FST_DROP_FRAME) {
		for (subFrm = 0; subFrm < frmPeriod; subFrm++) {
			...
			if (IspInfo.TstpQInfo[module].DmaEnStatus[_imgo_]) {
				ISP_PushBufTimestamp(module, _imgo_,
					sec, usec, frmPeriod);
			}

			if (IspInfo.TstpQInfo[module].DmaEnStatus[_rrzo_]) {
				ISP_PushBufTimestamp(module, _rrzo_,
					sec, usec, frmPeriod);
			}

			if (IspInfo.TstpQInfo[module].DmaEnStatus[_ufeo_]) {
				ISP_PushBufTimestamp(module, _ufeo_,
					sec, usec, frmPeriod);
			}

			if (IspInfo.TstpQInfo[module].DmaEnStatus[_ufgo_]) {
				ISP_PushBufTimestamp(module, _ufgo_,
					sec, usec, frmPeriod);
			}

			if (IspInfo.TstpQInfo[module].DmaEnStatus[_lmvo_]) {
				ISP_PushBufTimestamp(module, _lmvo_,
					sec, usec, frmPeriod);
			}

			if (IspInfo.TstpQInfo[module].DmaEnStatus[_lcso_]) {
				ISP_PushBufTimestamp(module, _lcso_,
					sec, usec, frmPeriod);
			}
		}
		...
		unsigned int usec, unsigned int frmPeriod)
{
	...
	case ISP_IRQ_TYPE_INT_CAM_C_ST:
		...
		case _ufeo_:
			fbc_ctrl2.Raw = ISP_RD32(
				CAM_REG_FBC_UFEO_CTL2(reg_module));
			break;
		case _lmvo_:
			fbc_ctrl2.Raw = ISP_RD32(
				CAM_REG_FBC_LMVO_CTL2(reg_module));
			break;
		case _lcso_:
			fbc_ctrl2.Raw = ISP_RD32(
				CAM_REG_FBC_LCSO_CTL2(reg_module));
			break;
		case _rsso_:
			fbc_ctrl2.Raw = ISP_RD32(
				CAM_REG_FBC_RSSO_CTL2(reg_module));
			break;
		case _aao_:
			fbc_ctrl2.Raw = ISP_RD32(
	...
	IspInfo.TstpQInfo[module].Dmao[
		dma_id].TimeQue[wridx].sec = sec;
	IspInfo.TstpQInfo[module].Dmao[
		dma_id].TimeQue[wridx].usec = usec;

	if (IspInfo.TstpQInfo[module].Dmao[
		dma_id].WrIndex >= (ISP_TIMESTPQ_DEPTH-1)) {
	...
	IspInfo.TstpQInfo[module].Dmao[dma_id].PrevFbcWCnt =
		((IspInfo.TstpQInfo[module].Dmao[
			dma_id].PrevFbcWCnt + 1) & 0x3F);

	return 0;
}

static int32_t ISP_PopBufTimestamp(unsigned int module,
	unsigned int dma_id, struct S_START_T *pTstp)
{
	switch (module) {
	case ISP_IRQ_TYPE_INT_CAM_A_ST:
	case ISP_IRQ_TYPE_INT_CAM_B_ST:
	...
	if (pTstp) {
		*pTstp = IspInfo.TstpQInfo[module].Dmao[
			dma_id].TimeQue[IspInfo.TstpQInfo[
					module].Dmao[dma_id].RdIndex];
	}

	if (IspInfo.TstpQInfo[module].Dmao[
		dma_id].RdIndex >= (ISP_TIMESTPQ_DEPTH-1)) {
		IspInfo.TstpQInfo[module].Dmao[
			dma_id].RdIndex = 0;
	} else {
		IspInfo.TstpQInfo[module].Dmao[
...
static int32_t ISP_WaitTimestampReady(unsigned int
	module, unsigned int dma_id)
{
	unsigned int _timeout = 0;
	unsigned int wait_cnt = 0;

	if (IspInfo.TstpQInfo[module].Dmao[
		dma_id].TotalWrCnt > IspInfo.TstpQInfo[
	...
	for (wait_cnt = 3; wait_cnt > 0; wait_cnt--) {
		...
		/* check if user is interrupted by system signal */
		if ((_timeout != 0) && (!(IspInfo.TstpQInfo[
			module].Dmao[dma_id].TotalWrCnt >
				IspInfo.TstpQInfo[module].Dmao[
					dma_id].TotalRdCnt))) {
			LOG_INF(
...
static int32_t ISP_CompensateMissingSofTime(
		enum ISP_DEV_NODE_ENUM reg_module,
		...
		unsigned int usec, unsigned int frmPeriod)
{
	...
	if (IspInfo.TstpQInfo[module].Dmao[
		...
		delta_wcnt = fbc_ctrl2.Bits.WCNT + 64 - IspInfo.TstpQInfo[
			module].Dmao[dma_id].PrevFbcWCnt;
	} else {
		delta_wcnt = fbc_ctrl2.Bits.WCNT - IspInfo.TstpQInfo[
			module].Dmao[dma_id].PrevFbcWCnt;
	}
	...
	wridx = IspInfo.TstpQInfo[module].Dmao[
		dma_id].WrIndex;
	wridx_prev1 = (wridx == 0) ? (
		ISP_TIMESTPQ_DEPTH - 1) : (wridx - 1);
	wridx_prev2 = (wridx_prev1 == 0) ? (
		ISP_TIMESTPQ_DEPTH - 1) : (wridx_prev1 - 1);

	time_prev1.sec = IspInfo.TstpQInfo[module].Dmao[
		dma_id].TimeQue[wridx_prev1].sec;
	time_prev1.usec = IspInfo.TstpQInfo[module].Dmao[
		dma_id].TimeQue[wridx_prev1].usec;

	time_prev2.sec = IspInfo.TstpQInfo[module].Dmao[
		dma_id].TimeQue[wridx_prev2].sec;
	time_prev2.usec = IspInfo.TstpQInfo[module].Dmao[
		dma_id].TimeQue[wridx_prev2].usec;

	if ((sec > time_prev1.sec) ||
		((sec == time_prev1.sec) &&
			(usec > time_prev1.usec))) {
		max_delta_time = ((sec - time_prev1.sec)*
	...
	for (i = 0; i < delta_wcnt; i++) {
		...
		ISP_PushBufTimestamp(module, dma_id,
			time_prev1.sec, time_prev1.usec, frmPeriod);
	}
	...
	if (dmao_mask) {
		IRQ_LOG_KEEPER(module, m_CurrentPPB, _LOG_INF,
			...
			IspInfo.TstpQInfo[module].Dmao[
				dma_id].PrevFbcWCnt,
			fbc_ctrl2.Bits.WCNT, sec, usec,
			delta_time, time_prev1.sec,
			time_prev1.usec);
	}
...
#if (TSTMP_SUBSAMPLE_INTPL == 1)
static int32_t ISP_PatchTimestamp(unsigned int module,
	unsigned int dma_id,
	unsigned int frmPeriod,
	unsigned long long refTimestp,
	unsigned long long prevTimestp)
{
	unsigned long long prev_tstp = prevTimestp;
	unsigned long long cur_tstp = refTimestp;
	...
	curr_wridx = IspInfo.TstpQInfo[
		module].Dmao[dma_id].WrIndex;

	if (curr_wridx < frmPeriod) {
		target_wridx = (
			curr_wridx + ISP_TIMESTPQ_DEPTH - frmPeriod);
	} else {
		target_wridx = curr_wridx - frmPeriod;
	}

	frm_dt = (((unsigned int)(
		cur_tstp - prev_tstp)) / frmPeriod);
	last_frm_dt = ((
...
{
	return ISP_Irq_CAMSV(ISP_IRQ_TYPE_INT_CAMSV_0_ST,
		ISP_CAMSV0_IDX, "CAMSV0");
}

irqreturn_t ISP_Irq_CAMSV_1(int  Irq, void *DeviceId)
{
	return ISP_Irq_CAMSV(ISP_IRQ_TYPE_INT_CAMSV_1_ST,
		ISP_CAMSV1_IDX, "CAMSV1");
}

irqreturn_t ISP_Irq_CAMSV_2(int  Irq, void *DeviceId)
{
	return ISP_Irq_CAMSV(ISP_IRQ_TYPE_INT_CAMSV_2_ST,
		ISP_CAMSV2_IDX, "CAMSV2");
}

irqreturn_t ISP_Irq_CAMSV_3(int  Irq, void *DeviceId)
{
	return ISP_Irq_CAMSV(ISP_IRQ_TYPE_INT_CAMSV_3_ST,
		ISP_CAMSV3_IDX, "CAMSV3");
}

irqreturn_t ISP_Irq_CAMSV_4(int  Irq, void *DeviceId)
{
	return ISP_Irq_CAMSV(ISP_IRQ_TYPE_INT_CAMSV_4_ST,
		ISP_CAMSV4_IDX, "CAMSV4");
}

irqreturn_t ISP_Irq_CAMSV_5(int  Irq, void *DeviceId)
{
	return ISP_Irq_CAMSV(ISP_IRQ_TYPE_INT_CAMSV_5_ST,
		ISP_CAMSV5_IDX, "CAMSV5");
}
	...
	enum ISP_DEV_NODE_ENUM cam_idx, const char *str)
{
	...
	unsigned int i, IrqStatus, ErrStatus;
	unsigned int time_stamp, cur_v_cnt = 0;
	...
	IrqStatus = ISP_RD32(
		CAMSV_REG_INT_STATUS(reg_module));
	spin_unlock(&(IspInfo.SpinLockIrq[module]));

	ErrStatus = IrqStatus & IspInfo.IrqInfo.ErrMask[
		module][SIGNAL_INT];
	IrqStatus = IrqStatus & IspInfo.IrqInfo.Mask[
		module][SIGNAL_INT];

	/* Check ERR/WRN ISR times,
	 * if it occur too frequently,
	 * mark it for avoding keep enter ISR
	 * It will happen KE
	 */
	for (i = 0; i < ISP_ISR_MAX_NUM; i++) {
		/* Only check irq that un marked yet */
		if (!(IspInfo.IrqCntInfo.m_err_int_mark[
			module] & (1 << i))) {
	...
	spin_lock(&(IspInfo.SpinLockIrq[module]));
	IrqEnableOrig = ISP_RD32(
		CAMSV_REG_INT_EN(reg_module));
	spin_unlock(&(IspInfo.SpinLockIrq[module]));
	...
	fbc_ctrl1[0].Raw = ISP_RD32(
		CAMSV_REG_FBC_IMGO_CTL1(reg_module));
	fbc_ctrl2[0].Raw = ISP_RD32(
		CAMSV_REG_FBC_IMGO_CTL2(reg_module));
	time_stamp       = ISP_RD32(
		CAMSV_REG_TG_TIME_STAMP(reg_module));
	...
	if ((IrqStatus & SV_HW_PASS1_DON_ST) || (
		 ...
		 */
		cur_v_cnt = ISP_RD32_TG_CAM_FRM_CNT(
			module, reg_module);
	}
	...
	if (IrqStatus & SV_SW_PASS1_DON_ST) {
		 ...
		 */
		IspInfo.IrqInfo.LastestSigTime_usec[
			module][10] = (unsigned int)(usec);
		IspInfo.IrqInfo.LastestSigTime_sec[
			module][10] = (unsigned int)(sec);
	...
	if (IrqStatus & SV_SOF_INT_ST) {
		...
		if (IspInfo.DebugMask & ISP_DBG_INT) {
			...
			"%s P1_SOF_%d_%d(0x%08x_0x%08x,0x%08x),int_us:0x%08x, stamp[0x%08x]\n",
				...
				(unsigned int)(ISP_RD32(
				CAMSV_REG_FBC_IMGO_CTL2(reg_module))),
				ISP_RD32(CAMSV_REG_IMGO_BASE_ADDR(
				reg_module)),
...
irqreturn_t ISP_Irq_CAM(enum ISP_IRQ_TYPE_ENUM irq_module)
{
	unsigned int module = irq_module;
	unsigned int reg_module = ISP_CAM_A_IDX;
	unsigned int i, cardinalNum = 0, cur_v_cnt = 0;
	unsigned int IrqStatus, ErrStatus;
	unsigned int WarnStatus, DmaStatus, WarnStatus_2;
	...
	for (i = 0; i < ISP_ISR_MAX_NUM; i++) {
		...
		if (!((IspInfo.IrqCntInfo.m_err_int_mark[
			...
			if (usec - IspInfo.IrqCntInfo.m_int_usec[
				module] < INT_ERR_WARN_TIMER_THREAS) {
				if (IspInfo.IrqCntInfo.m_err_int_cnt[
					module][i] >= INT_ERR_WARN_MAX_TIME) {
					IspInfo.IrqCntInfo.m_err_int_mark[
						module] |= (1 << i);
				...
				if (IspInfo.IrqCntInfo.m_warn_int_cnt[
					module][i] >= INT_ERR_WARN_MAX_TIME) {
					IspInfo.IrqCntInfo.m_warn_int_mark[
						module] |= (1 << i);
				}
	...
	if ((IrqStatus & HW_PASS1_DON_ST) || (IrqStatus & SOF_INT_ST))
		/*cur_v_cnt =
		 * ((ISP_RD32(CAM_REG_TG_INTER_ST(reg_module))
		 * & 0x00FF0000) >> 16);
		 */
	...
	if (IrqStatus & SW_PASS1_DON_ST) {
		...
		do_div(sec, 1000);	  /* usec */
		usec = do_div(sec, 1000000);	/* sec and usec */
		/* update pass1 done time stamp for
		 * eis user(need match with the time stamp in image header)
		 */
		IspInfo.IrqInfo.LastestSigTime_usec[
			module][10] = (unsigned int)(usec);
		IspInfo.IrqInfo.LastestSigTime_sec[
	...
	if (IrqStatus & SOF_INT_ST) {
		...
		if (IspInfo.DebugMask & ISP_DBG_INT) {
			...
			"CAM%c P1_SOF_%d_%d(0x%x_0x%x,0x%x_0x%x,0x%x,0x%x,0x%x),int_us:%d,cq:0x%x,dbg:0x%x\n",
					...
					(unsigned int)(ISP_RD32
					(CAM_REG_FBC_RRZO_CTL2(reg_module))),
					ISP_RD32(CAM_REG_IMGO_BASE_ADDR(
						reg_module)),
					ISP_RD32(CAM_REG_RRZO_BASE_ADDR(
						reg_module)),
					magic_num,
					(unsigned int)((
						sec * 1000000 + usec) - (
			...
			"CAM%c_aa(0x%x_0x%x_0x%x) af(0x%x_0x%x_0x%x),pd(0x%x_0x%x_0x%x),ps(0x%x_0x%x_0x%x)\n",
					...
					(unsigned int)(ISP_RD32
					(CAM_REG_FBC_AAO_CTL2(reg_module))),
					ISP_RD32(CAM_REG_AFO_BASE_ADDR(
						reg_module)),
					(unsigned int)(ISP_RD32
					(CAM_REG_FBC_AFO_CTL1(reg_module))),
					(unsigned int)(ISP_RD32
					(CAM_REG_FBC_AFO_CTL2(reg_module))),
					ISP_RD32(CAM_REG_PDO_BASE_ADDR(
						reg_module)),
					(unsigned int)(ISP_RD32
					(CAM_REG_FBC_PDO_CTL1(reg_module))),
					(unsigned int)(ISP_RD32
					(CAM_REG_FBC_PDO_CTL2(reg_module))),
					ISP_RD32(CAM_REG_PSO_BASE_ADDR(
						reg_module)),
					(unsigned int)(ISP_RD32
		 ...
		 */
		IspInfo.IrqInfo.LastestSigTime_usec[
			module][12] = (unsigned int)(sec);
		IspInfo.IrqInfo.LastestSigTime_sec[
			module][12] = (unsigned int)(usec);
